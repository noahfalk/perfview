...
enum TypeCode
{
  ...
  LengthPrefixedUTF16String = 22,    // A string encoded with UTF16 characters and a UInt16 element count prefix. No null-terminator.
  LengthPrefixedUTF8String = 23,     // A string encoded with UTF8 characters and a Uint16 length prefix. No null-terminator.
  VarLengthPrefixedUtf8String = 24,  // A string encoded with UTF8 characters and a Varuint length prefix. No null-terminator.
}
```
...
- Concatenated sequence of Count optional metadata elements, each of which is:
  ...
  - if Kind & LengthPrefixed != 0    // An extensibility mechanism so future versions of the file format can add new kinds of optional metadata
    - varuint Size                   // Encodes the size of the optional metadata element, not including Kind and Size fields
                                     // The reader should skip Size bytes of data to get to the next optional metadata element
